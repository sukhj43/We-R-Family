...
rsconnect::setAccountInfo(name='sj43', 
                          token='7D7D1007FB9F28E25356DEF39F21A4C6', 
                          secret='9bduev0LNEgclZMUZvkriL5LfWL8+hJshMYe5j2K')
library(rsconnect)
rsconnect::deployApp()
deployApp('E:/R',account = 'sj43')
...
dd <- sqldf("
select CNTY, CHILD_DEATH_YR,total_count_yr, sum(CNT_BY_UNDERLYING_COD) as death_count
, (sum(CNT_BY_UNDERLYING_COD)*100/total_count_yr) as cnty_percent_yr
from(

  SELECT CNTY, CHILD_DEATH_YR, UNDERLYING_COD,CNT_BY_UNDERLYING_COD
  , sum(CNT_BY_UNDERLYING_COD) over(partition by CHILD_DEATH_YR ) as total_count_yr
  -- , (sum(CNT_BY_UNDERLYING_COD) / (sum(CNT_BY_UNDERLYING_COD) over(partition by CHILD_DEATH_YR ))) as cnty_percent_yr
  FROM county_data
  GROUP BY CNTY, CHILD_DEATH_YR,UNDERLYING_COD,CNT_BY_UNDERLYING_COD)
  group by CNTY, CHILD_DEATH_YR,total_count_yr
          ")

dd

dd$CNTY <- tolower(dd$CNTY)
...
ui <- fluidPage(
  ...
  navbarPage("Navbar",
             ...
             tabPanel("Prediction Proability", mainPanel(dataTableOutput("Pred")),
                      fluidRow(
                        column(3,
